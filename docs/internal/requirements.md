...
### 12.1 保留事項: iRIClib APIのみでの統合
- 現時点では保留とする。
- 理由:
  - 分割CGNSを汎用的に統合するには、結果変数名・配置（Node/Cell/IFace/JFace）・対象ソルバーの出力仕様を事前に確定する必要がある。
  - 現行実装のような `ZoneIterativeData` のポインタ群を含む汎用コピーは、APIのみでの置換コストが高い。
- 再開条件:
  - 対象ソルバーを限定し、取り込み対象の結果項目と配置を固定できること。
  - API-onlyで不足する項目がある場合は、`h5py` 併用のハイブリッド方針を許容すること。
